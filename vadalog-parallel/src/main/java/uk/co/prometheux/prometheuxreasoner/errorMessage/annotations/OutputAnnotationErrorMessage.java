...
public class OutputAnnotationErrorMessage implements AnnotationErrorMessage {
	...
	public OutputAnnotationErrorMessage() {}
	
	public OutputAnnotationErrorMessage(Atom input) {
		vadalogProgramAsRuleList = SyntaxErrorMessageIdentifier.getVadalogProgramAsList();
		this.lineNumber = this.getRuleLineFromAtom(input);
		String wrongString = input.getName();
		this.columnNumber = input.toClauseString().indexOf(wrongString);
	...
	private void structurateMessage() {
		...
		this.structuredInfo += ": Line " + (this.lineNumber + 1);
