...
public class StrongLinksProgramEvaluator extends ProgramEvaluator {
	...
	private String strongLinksOutputPath;
	...
	public StrongLinksProgramEvaluator(List<BindAnnotation> inputAnnotations, List<BindAnnotation> outputAnnotations) {
		...
		BindAnnotation outputStrongLinks = outputAnnotations.get(0);

		strongLinksOutputPath = outputStrongLinks.getSchema() + File.separator + outputStrongLinks.getTableOrQuery();
	}

	@Override
	public void evaluate() {
		System.out.println("STRONG LINKS, reading input file " + new File(keyPersonInputPath).getAbsolutePath());
		Dataset<Row> keyPerson = spark.read().format("csv").option("header", "true").option("inferSchema", "true")
				.load(this.keyPersonInputPath);

		System.out.println("STRONG LINKS, reading input file " + new File(personInputPath).getAbsolutePath());
		Dataset<Row> person = spark.read().format("csv").option("header", "true").option("inferSchema", "true")
				.load(this.personInputPath);

		System.out.println("STRONG LINKS, reading input file " + new File(companyInputPath).getAbsolutePath());
		Dataset<Row> company = spark.read().format("csv").option("header", "true").option("inferSchema", "true")
				.load(this.companyInputPath);

		System.out.println("STRONG LINKS, reading input file " + new File(controlInputPath).getAbsolutePath());
		...
		Dataset<Row> strongLinks = this.withFlatMapGroup(keyPerson, company);
		strongLinks.write().mode(SaveMode.Overwrite).csv(strongLinksOutputPath);

		System.out.println("STRONG LINKS, written output file " + new File(strongLinksOutputPath).getAbsolutePath());
	...
	private Dataset<Row> withFlatMapGroup(Dataset<Row> keyPerson, Dataset<Row> company) {
		...
		Encoder<Row> strongLinksEncoder = EncoderGenerator.generateDefaultEncoder("psc", 4, datatypes);


		StrongLinksCoGroup strongLinksCoGroup = new StrongLinksCoGroup(bControl, bPerson);

		Dataset<Row> strongLinks = groupedKP.cogroup(groupedCompany, strongLinksCoGroup, strongLinksEncoder);
		return strongLinks;
	}
